...
export default function ProductDetails({ product }) {
  return (
    <ProductOptionsProvider data={product}>
      <section className="w-full overflow-x-hidden gap-4 md:gap-8 grid px-6 md:px-8 lg:px-12 bg-background-main">
        <div className="grid items-start gap-6 lg:gap-20 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="sticky md:mx-auto max-w-xl md:max-w-[24rem] grid gap-8 p-0 md:p-6 md:px-0 top-[6rem] lg:top-[8rem] xl:top-[10rem]">
            <div className="grid gap-2">
              <h1 className="text-4xl font-bold leading-10 whitespace-normal text-text-color-main">
                {product.title}
              </h1>
              <span className="max-w-prose whitespace-pre-wrap inherit text-copy opacity-50 font-medium text-text-color-main">
                {product.vendor}
              </span>
            </div>
            ...
            <div className="mt-8">
              <div
                className="prose border-t border-gray-200 pt-6 text-text-color-main text-md"
  ...
  return (
    <form className="grid gap-10">
      ...
      <div>
        ...
        />
        <ProductPrice
          className="text-text-color-main text-lg font-semibold"
          variantId={selectedVariant.id}
          data={product}
        />
      </div>
...
function PurchaseMarkup() {
  ...
  return (
    <>
      ...
      {isOutOfStock ? (
        <span className="text-gray-500 text-center py-3 px-6 border rounded-sm leading-none ">
          Available in 2-3 weeks
        </span>
      ) : (
        <BuyNowButton variantId={selectedVariant.id}>
          <span className="inline-block rounded-sm font-medium text-center py-3 px-6 max-w-xl leading-none border w-full bg-section-main">
            Buy it now
          </span>
        </BuyNowButton>
      )}
